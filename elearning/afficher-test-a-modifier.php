  ...
  include("menu-ens.php");
  	 //
		  ...
		  if(isset($_GET["te"]))
            ...
            $titre="";
?>
	<div class="container"  style="margin-top:20px;width:50%;float:clear;color: black;">
		<h4 style="text-align:center;color: white;">Test : <?php echo $titre; ?></h4>
		<br/> 
      ...
      <?php
            ...
            try
            {
			   ...
			   //
                ...
                echo "<tr><td>Num.</td><td>Question</td><td>Bareme</td><td>Action1</td><td>Action2</td></tr>";
                //
				foreach($res as $ligne)
                {
			        ...
			        foreach($res2 as $ligne2)
                    {
					   ...
					   echo "<table width='400'><tr><td width='20'>$numr</td><td width='200'>$libr</td><td width='30'>(<span style='color:#0f0;'>$barr</span>)
					   </td><td width='100'><a href=\"modifier-reponse.php?ir=$idr&re=$libr\" target='_blank'>Modifier</a></td></tr></table>";
				    }
					echo "</td><td>$baremeq</td><td><a href=\"modifier-question.php?iq=$idq&qe=$libelleq\" target='_blank'>Modifier</a></td>
					<td><a href='#'  onClick='supprimerq(\"supprimer-question.php?iq=$idq\")'>Supprimer</a></td></tr>";
                }
               //
               //
               }
	...
	//
    function supprimerr(url)
     {
      if (confirm("Etes-vous certain(e) de vouloir supprimer cette r&eacute;ponse?"))
      {
         window.open(url,"suppression","menubar=no, status=no, scrollbars=yes, menubar=no, width=850, height=700");
      }
      else
...
<style >
  body{
      background: linear-gradient(rgba(0,0,0,0.6),rgba(0,0,0,0.6)), url("pictures/imge3.jpg");
      height: 100vh;
      width: 98%;
