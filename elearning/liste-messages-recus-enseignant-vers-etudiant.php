...
try
              {
			   ...
			   $iduser=0;
               //
               $res=$pdo->prepare("select * from etudiant where id_user= :par");
			   //	   
...
$pass = "";
              try
              {
               ...
               //
               $res=$pdo->query("select m.*,e.nom_enseignant,e.prenom_enseignant from message m, enseignant e where m.id_expediteur=e.id_user and m.type_expediteur like 'ens%' and m.type_destinataire like 'etu%' and (m.id_destinataire=$ida or m.id_destinataire='-99') order by m.date desc;");
                $res->setFetchMode(PDO::FETCH_ASSOC);
               //
			     $ctr=0;
			   //
